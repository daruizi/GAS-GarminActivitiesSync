...
- **兜底不能省，但也不用太频繁**：webhook 投递不是 100% 可靠（Strava 官方文档写明非 2xx 最多重试 3 次就放弃），完全依赖事件驱动会有漏网风险。上线首日先用了每日一次，实测发现过于保守；调整为每 3 小时一次（`cron: "30 */3 * * *"`）——相比原来的每小时轮询已经降到 1/3 的运行次数，同时把"漏网"的后果从"可能永久漏掉"收窄到大致 3 小时量级。**但这个"3 小时"只是 cron 间隔，不是真实的最坏延迟保证**：GitHub 自己的 `schedule` 触发不保证准时，2026-07-12 上线当天实测过 36 分钟和 84 分钟的额外延迟，真实最坏情况要在 3 小时基础上再加上 GitHub 当时的调度积压，详见下方故障模式表。
...
| 故障 | 影响 | 缓解 |
|---|---|---|
| **GitHub 自己的 `schedule` 触发延迟**（实测存在，非本仓库可控）| cron 设的是每 3 小时，但 GitHub 不保证准时触发——2026-07-12 实测过 36 分钟和 84 分钟的延迟（`:30` 错峰也没能完全避免）。也就是说文档里"最坏等 3 小时"的说法偏乐观，真实最坏情况是"3 小时 + GitHub 当时的调度积压"，理论上可能到 4 小时甚至更久 | 无法从我们这端修复（GitHub Actions 免费层的已知限制，官方文档承认高负载时会延迟）。事件驱动主链路完全不受影响——今天所有 `repository_dispatch` 触发的运行都是秒级响应；只有"漏网兜底"这一层会被拖慢。如果对兜底时效要求高，可以用 `gh run list -e schedule` 定期抽查实际延迟，或改用更短的 cron 间隔换取更小的绝对延迟占比 |
| Strava 事件丢失/未送达 | 该活动延迟同步 | 每 3 小时兜底扫最近 10 条活动（叠加上面这条 GitHub 调度延迟后，实际到达时间会更晚） |
